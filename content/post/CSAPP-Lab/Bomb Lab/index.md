...
即从1开始，往后每个数都是前一个数的2倍，即a0=1, p=2, N=6的等比数列{1, 2, 4, 8, 16, 32}
...
由于使用4位二进制数作为偏移量，所以最多可以访问到该字符串的前64个字符
...
可以看到源字符串是前面读入的6个字符串中的第四个，即***phase_4***，格式化字符串读入两个数和一个字符串。
...
简单分析一下可以看出，这段代码主要是递归访问二叉树，因此为了分析具体逻辑对该段代码进行反编译
...
因此最终二叉树的遍历路径为先访问左分支，再访问右分支，然后使num与该分支的数据域相等即可。
